...
def ejecutar():
    # 🎨 INYECCIÓN ESTÉTICA PREMIUM (GÉNESIS GOLDEN CONTOURS)
    st.markdown("""
        <style>
        .titulo-genesis {
            color: #0d1b2a;
    ...
    # Cabecera de Comando Central
    st.markdown("<p class='titulo-genesis'>⚙️ Creador de Plantillas Maestras</p>", unsafe_allow_html=True)
    st.markdown("<p class='subtitulo-genesis'>Módulo de Configuración Óptica Avanzada v2.5 Oficial</p>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    # 🏢 Formulario en Cuadrícula Segura 2x3 (Evita bloqueos de renderizado)
    with st.container(border=True):
        # 📡 FILA 1: Parametrización Básica
        f1_c1, f1_c2, f1_c3 = st.columns(3)
        with f1_c1:
            nombre_examen = st.text_input("🎯 Nombre de la Evaluación:", placeholder="Ej: Primer bimestral Periodo")
        with f1_c2:
            listado_materias = [
        ...
        with f1_c3:
            listado_tipos = [
                ...
                "Quiz", "Evaluación Primer Periodo", "Evaluación Segundo Periodo",
                "Evaluación Tercer Periodo", "Evaluación Cuarto Periodo", "Prepruebas Saber Pro"
            ]
            tipo_evaluacion = st.selectbox("📋 Tipo de Evaluación:", listado_tipos)
            
        # 📡 FILA 2: Telemetría y Segmentación Escolar
    ...
    # Botón de registro masivo con validaciones completas de 4 puntos
    if st.button("💾 GUARDAR CONFIGURACIÓN Y CREAR PLANTILLA", use_container_width=True, type="primary"):
        ...
        if tipo_evaluacion == "--- Seleccione Tipo de Evaluación ---":
            st.error("❌ Por favor, especifique el tipo de evaluación para las ponderaciones.")
            return
